...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<List<String>> _androidFilePicker(FileSelectorParams params) async {
    ...
    if (choice == ImageSource.camera) {
      // Xử lý chụp ảnh (tương tự như trên)
    } else if (choice == null) {
      ...
      if (Platform.isAndroid) {
        if (pickerParams.fileType == FileType.image) {
          ...
          permissionNeeded = Permission.camera;
        }
      } else {
        permissionNeeded = Permission.photos;
      }
      ...
      PermissionStatus fileAccessStatus =
        ...
        "Ứng dụng cần quyền truy cập $permissionFriendlyName để bạn chọn tệp.",
      );
      ...
      } else if (fileAccessStatus.isPermanentlyDenied) {
        await _showPermissionPermanentlyDeniedDialog(
